...
public class ChartDataServiceImpl implements ChartDataService {
    ...
    public WeightBmiTrendVO getWeightBmiTrend(Long userId, Integer days) {
        ...
        if (weightHeight == null || weightHeight.get("weightKg") == null || weightHeight.get("heightCm") == null) {
            // 没有数据，返回空结果
            vo.setDataPoints(new ArrayList<>());
            return vo;
        }
        
        BigDecimal currentWeight = weightHeight.get("weightKg");
        ...
        BigDecimal currentBmi = currentWeight.divide(heightM.multiply(heightM), 2, RoundingMode.HALF_UP);
        
        vo.setCurrentWeight(currentWeight);
        vo.setCurrentBmi(currentBmi);
        vo.setBmiStatus(getBmiStatus(currentBmi));
        
        // 生成模拟趋势数据（因为user_profile表只有当前值）
        // 实际应用中应该有weight_history表记录历史体重
        List<WeightBmiTrendVO.DataPoint> dataPoints = generateWeightTrendData(currentWeight, currentBmi, currentHeight, days);
     ...
     */
    private List<WeightBmiTrendVO.DataPoint> generateWeightTrendData(BigDecimal currentWeight, BigDecimal currentBmi, BigDecimal heightCm, Integer days) {
    ...
    public NutritionDistributionVO getNutritionDistribution(Long userId, LocalDate date) {
        ...
        if (totalGrams.compareTo(BigDecimal.ZERO) > 0) {
            macroDistribution.setProteinPercentage(
                protein.divide(totalGrams, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100")).setScale(1, RoundingMode.HALF_UP)
            );
            macroDistribution.setCarbPercentage(
                carb.divide(totalGrams, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100")).setScale(1, RoundingMode.HALF_UP)
            );
            macroDistribution.setFatPercentage(
                fat.divide(totalGrams, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100")).setScale(1, RoundingMode.HALF_UP)
            );
        }
        ...
        mealDistribution.setBreakfast(record.getBreakfastCalories() != null ? record.getBreakfastCalories() : BigDecimal.ZERO);
        ...
        List<UserNutritionRecord> recentRecords = userNutritionRecordMapper.selectByUserIdAndDateRange(userId, startDate, date);
    ...
    public CalorieBurnTrendVO getCalorieBurnTrend(Long userId, Integer days) {
        ...
        // 查询营养记录
        List<UserNutritionRecord> records = userNutritionRecordMapper.selectByUserIdAndDateRange(userId, startDate, endDate);
